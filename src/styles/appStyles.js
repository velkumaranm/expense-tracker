...
.charts-grid{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:12px;margin-bottom:16px;align-items:start;grid-auto-flow:row dense}
...
.chart-card{padding:18px;display:flex;flex-direction:column;justify-content:flex-start;height:auto;align-self:start}
...
.chart-legend-compact{display:flex;align-items:center;justify-content:center;gap:12px;flex-wrap:wrap;margin-top:8px}
.chart-legend-compact span{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:600;color:var(--text2);line-height:1.2;white-space:nowrap}
.chart-legend-compact i{width:9px;height:9px;border-radius:2px;display:inline-block;flex-shrink:0}
.pie-row{display:flex;align-items:center;gap:12px;min-width:0}
...
.mini-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,160px),1fr));gap:10px;align-items:stretch}
.mini-card{background:var(--card2);border:1px solid var(--border);border-radius:14px;padding:12px;min-width:0;overflow:hidden}
.mini-card .v{font-size:20px;font-family:'Cormorant Garamond',serif;font-weight:700}
.mini-card .k{font-size:10px;text-transform:uppercase;letter-spacing:1px;color:var(--text3);margin-bottom:5px;overflow-wrap:anywhere}
.mini-card .muted{overflow-wrap:anywhere}
.dashboard-mini-grid{grid-template-columns:repeat(3,minmax(0,1fr))}
.dashboard-fixed-card{height:250px;overflow:hidden}
.dashboard-fixed-card .insight-list,.dashboard-fixed-card .dashboard-stat-list{min-height:0;overflow:auto;padding-right:5px}
.dashboard-attention-card .insight-list{max-height:176px}
.dashboard-diagnostics-card{justify-content:flex-start}
.dashboard-diagnostics-card .dashboard-mini-grid{height:100%}
.dashboard-diagnostics-card .mini-card{min-height:0;height:166px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}
.dashboard-diagnostics-card .mini-card .k{min-height:32px;display:flex;align-items:center;justify-content:center;text-align:center}
.dashboard-diagnostics-card .mini-card .muted{max-height:58px;overflow:auto;padding-right:2px}
.dashboard-list-card{min-height:0}
.dashboard-stat-list{display:grid;gap:2px;max-height:220px;overflow:auto;padding-right:4px}
.dashboard-fixed-card.dashboard-list-card .dashboard-stat-list{max-height:174px}
.dashboard-list-card .stat-line span:first-child{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.dashboard-fixed-card.dashboard-bento-card{height:290px}
.dashboard-bento-card .mini-grid{min-height:0;overflow:auto;padding-right:4px}
.dashboard-bento-card .mini-card{min-height:150px;max-height:180px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}
.dashboard-bento-card .mini-card .k{min-height:30px;display:flex;align-items:center;justify-content:center}
.dashboard-bento-card .mini-card .muted{max-height:62px;overflow:auto;padding-right:2px}
...
.premium-control-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-bottom:14px;align-items:stretch}
.premium-control-card{height:320px;margin-bottom:0;overflow:hidden}
.premium-card-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:12px;min-height:62px}
.premium-card-head h3{margin-bottom:4px}
.premium-card-head p{font-size:11px;color:var(--text3);line-height:1.45;margin:0;max-height:48px;overflow:auto}
.trust-list,.smart-rule-list{display:grid;gap:7px;min-height:0;overflow:auto;padding-right:4px}
.trust-list{max-height:150px}
.trust-row{display:flex;align-items:center;justify-content:space-between;gap:10px;border-bottom:1px solid var(--border);padding:8px 0;font-size:11px;color:var(--text2)}
.trust-row:last-child{border-bottom:none}
.trust-row strong{font-size:11.5px;color:var(--text);text-align:right}
.review-action-list{display:grid;gap:7px;max-height:118px;min-height:0;overflow:auto;padding-right:4px;margin-bottom:8px}
.review-action-row{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:8px;background:var(--card2);border:1px solid var(--border);border-radius:12px;padding:9px 10px;text-align:left}
.review-action-row.warn{border-color:rgba(255,107,107,.32);background:rgba(255,107,107,.06)}
.review-action-row.pending,.review-action-row.info{border-color:rgba(200,169,110,.28);background:rgba(200,169,110,.07)}
.review-action-copy{display:grid;gap:3px;min-width:0}
.review-action-copy strong{font-size:11.5px;color:var(--text);line-height:1.3;overflow-wrap:anywhere}
.review-action-copy small{font-size:10px;color:var(--text3);line-height:1.35;overflow-wrap:anywhere}
.review-action-empty{border:1px dashed var(--border);border-radius:12px;padding:14px 12px;text-align:center;color:var(--text3);font-size:11.5px;line-height:1.45}
.review-chip-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;max-height:154px;overflow:auto;padding-right:4px}
.review-chip-grid.compact{max-height:84px}
.review-chip-grid .mini-stat{min-height:78px}
.control-card-action{margin-top:auto;align-self:flex-start}
.control-actions-row{display:flex;gap:8px;flex-wrap:wrap;margin-top:auto}
.control-actions-row .btn-primary:disabled{opacity:.45;cursor:not-allowed;transform:none}
.smart-rule-list{max-height:150px}
.smart-rule-row{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:8px;background:var(--card2);border:1px solid var(--border);border-radius:12px;padding:9px 10px}
.smart-rule-copy{display:grid;gap:3px;min-width:0}
.smart-rule-row span{font-size:11.5px;color:var(--text);line-height:1.35;overflow-wrap:anywhere}
.smart-rule-row small{font-size:10px;color:var(--text3)}
.smart-rule-row em{font-size:9.5px;color:var(--text3);font-style:normal;line-height:1.3;max-height:28px;overflow:auto}
.smart-rule-actions{display:flex;gap:6px;align-items:center;flex-wrap:wrap;justify-content:flex-end}
.mini-action-btn{border:1px solid var(--border);background:var(--surface);color:var(--text);border-radius:999px;padding:7px 10px;font-size:10px;font-weight:800;cursor:pointer;white-space:nowrap;transition:border-color .18s,transform .18s}
.mini-action-btn.quiet{background:transparent;color:var(--text3)}
.mini-action-btn:hover{border-color:var(--border-l);transform:translateY(-1px)}
.smart-rule-empty{border:1px dashed var(--border);border-radius:12px;padding:18px 12px;text-align:center;color:var(--text3);font-size:11.5px;line-height:1.5}
.portfolio-grid{display:grid;grid-template-columns:minmax(0,1.1fr) minmax(280px,.9fr);gap:16px;align-items:start}
.portfolio-side-stack{display:grid;gap:14px;align-content:start;min-width:0}
...
.portfolio-holdings-scroll{display:grid;gap:8px;max-height:clamp(460px,56vh,760px);overflow:auto;padding-right:6px;align-content:start}
.portfolio-holdings-empty{min-height:180px}
...
.portfolio-snapshot-grid{display:grid;grid-template-columns:1fr;gap:10px;max-height:360px;overflow:auto;padding-right:4px}
...
.vault-toolbar{display:grid;grid-template-columns:1fr;gap:12px;align-items:start}
.vault-toolbar-copy{max-width:none;text-align:left;line-height:1.65}
...
.stack > *{min-width:0}
.charts-grid > .stack{align-self:start;align-content:start}
...
.settings-section,.section-card{padding:20px;margin-bottom:12px;display:flex;flex-direction:column;justify-content:flex-start;height:auto;align-self:start}
...
.page-crash-card{min-height:260px;display:grid;align-content:center;justify-items:start;gap:12px;border-color:rgba(248,113,113,.34);background:linear-gradient(180deg,var(--card),var(--expense-dim))}
.page-crash-card h2{font-size:20px;margin:0;color:var(--text)}
.page-crash-card p{max-width:720px;color:var(--text2);line-height:1.55;margin:0}
.page-crash-card pre{width:100%;max-width:920px;max-height:140px;overflow:auto;white-space:pre-wrap;word-break:break-word;background:var(--surface);border:1px solid rgba(248,113,113,.22);border-radius:14px;padding:12px;color:var(--expense);font-size:12px}
...
.command-overlay{position:fixed;inset:0;z-index:1000;background:rgba(9,9,11,.56);backdrop-filter:blur(12px);display:grid;place-items:start center;padding-top:10vh}
.command-palette{width:min(620px,calc(100vw - 28px));max-height:min(680px,calc(100vh - 80px));overflow:auto;background:var(--card);border:1px solid var(--border-l);border-radius:20px;box-shadow:0 24px 80px rgba(0,0,0,.45);padding:16px}
.command-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:12px}
.command-head h3{font-size:15px;margin-bottom:4px}
.command-head p{font-size:11.5px;color:var(--text3);line-height:1.45;margin:0}
.command-list{display:grid;gap:8px}
.command-item{display:grid;gap:4px;text-align:left;background:var(--card2);border:1px solid var(--border);border-radius:14px;padding:13px 14px;color:var(--text);cursor:pointer;transition:border-color .16s,transform .16s}
.command-item:hover{border-color:var(--border-l);transform:translateY(-1px)}
.command-item strong{font-size:12.5px}
.command-item span{font-size:11px;color:var(--text3);line-height:1.45}
.command-foot{margin-top:12px;padding-top:12px;border-top:1px solid var(--border);font-size:10.5px;color:var(--text3)}
...
.ai-context-card,.ai-overview-card,.ai-insight-card{height:320px;overflow:hidden;display:flex;flex-direction:column}
.ai-context-card .stack,.ai-overview-card .stack,.ai-insight-card .insight-list,.ai-insight-card .stack{min-height:0;overflow:auto;padding-right:5px}
.ai-overview-card p{max-height:82px;overflow:auto}
...
@media(max-width:1180px){
  ...
  .portfolio-holdings-scroll{max-height:clamp(420px,52vh,680px)}
}
@media(max-width:900px){
  .summary-span-4,.summary-span-3,.summary-span-6,.chart-span-7,.chart-span-6,.chart-span-5,.chart-span-4{grid-column:span 12}
  .two-col,.three-col,.mini-grid,.heat-row,.ai-main-grid,.ai-chat-shell,.ai-insights-grid,.account-grid,.goal-metrics,.onboarding-grid,.planner-grid,.premium-control-grid,.portfolio-grid,.portfolio-subgrid,.timeline-grid{grid-template-columns:1fr}
  .dashboard-mini-grid{grid-template-columns:repeat(3,minmax(0,1fr))}
  .portfolio-holdings-scroll{max-height:62vh;overflow:auto;padding-right:4px}
  .portfolio-snapshot-grid{max-height:300px}
}
@media(max-width:768px){
  ...
  .dashboard-fixed-card,.dashboard-fixed-card.dashboard-bento-card,.ai-context-card,.ai-overview-card,.ai-insight-card{height:auto;max-height:none;min-height:0}
  .premium-control-card{height:auto;min-height:0}
  .premium-card-head{min-height:0}
  .review-chip-grid{grid-template-columns:1fr;max-height:240px}
  .smart-rule-row{grid-template-columns:1fr;align-items:start}
  .smart-rule-actions{justify-content:flex-start}
  .mini-action-btn{justify-self:start}
  .dashboard-diagnostics-card .mini-card{height:auto;min-height:132px}
  .chart-legend-compact{justify-content:flex-start}
