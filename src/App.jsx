...
function PriceCard({ name, symbol, price, changePct, change, unit, sector, isIndex, error }) {
  ...
  return (
    <div className={`mkt-card${isIndex?" idx":""}`}>
      ...
      <div className="mkt-sym">{symbol}</div>
      {sector && !isIndex && <div className="mkt-sector">{sector}</div>}
      {error ? (
        <div style={{ fontSize:10.5, color:"var(--text3)", marginTop:6 }}>Rate limited · Retry in 15 min</div>
      ) : price == null ? (
...
function Markets() {
  ...
  const [nseStocks, setNse]       = useState([]);
  const [bseStocks, setBse]       = useState([]);
  const [usStocks, setUs]         = useState([]);
  ...
  const refreshAll = useCallback(async () => {
    ...
    const results = await Promise.allSettled([
      ...
      loadBatch(TRENDING_NSE, setNse),
      loadBatch(TRENDING_BSE, setBse),
      loadBatch(TRENDING_US, setUs),
      loadBatch(COMMODITY_SYMBOLS, setCom),
      loadCrypto(),
    ]);
    if (results.some(r => r.status === "rejected" || r.value === false)) setFetchErr(true);
    setLast(new Date());
    setLoading(false);
  ...
  useEffect(() => {
    refreshAll();
    timer.current = setInterval(refreshAll, 12 * 60 * 60 * 1000);
  ...
  return (
    <>
      ...
      <div className="refresh-bar">
        <p>{lastUpdate ? `Updated ${lastUpdate.toLocaleTimeString("en-IN")} · Auto-refresh every 12 hours` : "Loading live data…"}</p>
        {fetchErr && <span style={{ fontSize:10.5, color:"var(--insure)" }}>⚠ Some data rate-limited</span>}
      </div>
          ...
          {srResults.length > 0 && (
            <>
              ...
              <div className="market-grid" style={{ marginBottom:18 }}>
                {srResults.map(s => <PriceCard key={s.symbol} name={s.name} symbol={s.symbol} price={s.price} changePct={s.changePct} change={s.change} unit="₹" sector={s.sector} error={s.error} />)}
              </div>
            </>
          )}
